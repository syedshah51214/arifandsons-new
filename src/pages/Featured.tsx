...
export default function Featured() {
  ...
  useEffect(() => {
    ...
    const loadFeaturedProjects = async () => {
      ...
      const projectList: ProjectImage[] = Object.entries(imageModules)
        .map(([path, module], idx) => {
          const pathParts = path.split('/');
          const filename = pathParts[pathParts.length - 1] || `Project ${idx}`;
        ...
        .filter(p => 
          p.alt.toLowerCase().includes('marla') && 
          !p.alt.toLowerCase().includes('under')
        );
    ...
    <div className="min-h-screen bg-gray-900 text-white overflow-x-hidden">
      ...
      <header className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        ...
        <p className="text-gray-400 text-base md:text-lg max-w-3xl">
          Explore our premium 5 Marla residential projects showcasing exceptional architecture, quality craftsmanship, and modern design. These are some of our most acclaimed projects in Lahore.
        </p>
      </header>
          ...
          <>
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                    <div className="w-full">
                      <p className="text-sm text-gray-300 truncate">5 Marla Residential</p>
            ...
            <div className="mt-12 text-center">
              <p className="text-gray-400">
                <span className="text-amber-400 font-semibold">{projects.length}</span> premium featured projects
              </p>
            </div>
          </>
        )}
          ...
          <div className="flex flex-col items-center justify-center max-w-4xl w-full gap-4">
            ...
            <div className="text-center w-full">
              ...
              <h2 className="text-2xl sm:text-3xl font-bold text-white mt-2">
                {projects[currentIndex]?.alt.split(' - ')[1] || 'Project'}
              </h2>
            </div>
